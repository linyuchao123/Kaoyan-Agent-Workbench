...
6. 用户可显式把仅含安全片段的分页索引缓存到当前浏览器 IndexedDB；缓存键由账户与资料编号组成，资料或分块版本变化时失效。
7. 查询优先匹配设备缓存，本地命中时不发送查询；无缓存或无命中时执行云端关键词 + 向量混合召回，通过 Reciprocal Rank Fusion 合并。
8. 云端检索只把通过相关性校验的片段送入模型，并强制输出 locator。
...
- `v1.4`（代码完成，待合并验收）：完成私有 PDF/Markdown 阅读器、账户隔离的浏览器索引缓存、版本失效、显式移除，以及本地关键词命中优先、无命中回退云端混合检索。
- `v1.5`（代码完成，待合并验收）：完成院校、资料与求职页的资料导师入口，按问题最小化加载当前用户上下文，按招生年份隔离院校档案，优先分析可行动求职记录，并在回答与历史中保留分类来源。
