...
> 当前状态：`v0.7` 已完成代码实现，等待云端迁移与真实密钥环境验收。登录、学习闭环、三级计划、错题复习、院校与求职、资料库、联网搜索、双 Agent、来源引用、确认入库、提案审批和对话恢复均已接通；Embedding 语义检索、OCR worker 与流式输出留待后续版本。
...
云端验收前需按文件名顺序执行 `202608130001` 至 `202608130005` 五个 v0.7 迁移，并配置可选的模型与 Tavily 密钥。

## 后续版本候选

- Embedding 与关键词融合排序，补齐真正的语义混合 RAG。
- 扫描 PDF OCR worker、任务队列和失败重试。
- Agent 流式输出、线程列表和主动新建对话。
