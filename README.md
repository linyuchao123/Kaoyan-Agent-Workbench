...
> 当前状态：`v1.4` 电子书与本地优先 RAG 已完成代码实现，`v1.3` 错题库与 AI 今日计划已完成云端验收。登录、任务、专注计时、手动补录、双指标年度热力图、三级计划、完整错题管理、AI 多任务提案、私有电子书、账户隔离的设备缓存、双 Agent、Embedding 混合检索、扫描 PDF OCR、来源引用、人工审批、幂等写入、审计日志、多轮对话恢复和 SSE 流式回答均已接通。
